...
describe('InputRouter', function () {
  before(function () {
    ...
    this.s6 = new InputSequence(DIRECTION_RIGHT)
    this.s6.register(DIRECTION_RIGHT)
    this.s4 = new InputSequence(DIRECTION_LEFT)
    this.s4.register(DIRECTION_LEFT)
    this.s5 = new InputSequence(DIRECTION_NEUTRAL)
  ...
  it('sorts sequences by priority', function () {
    ...
    expect(sequences[2].id).to.equal(DIRECTION_RIGHT)
    expect(sequences[3].id).to.equal(DIRECTION_LEFT)
    expect(sequences[4].id).to.equal(DIRECTION_NEUTRAL)
  })

  describe('#feed', function () {
    it('returns sequence on completion', function () {
      var router = this.router
      expect(router.feed(DIRECTION_NEUTRAL).id).to.equal(DIRECTION_NEUTRAL)
      expect(router.feed(DIRECTION_LEFT).id).to.equal(DIRECTION_LEFT)
      expect(router.feed(DIRECTION_RIGHT).id).to.equal(DIRECTION_RIGHT)
    ...
    context('when flipped', function () {
      ...
      it('flips commands horizontally', function () {
        var router = this.router
        expect(router.feed(DIRECTION_NEUTRAL).id).to.equal(DIRECTION_NEUTRAL)
        expect(router.feed(DIRECTION_LEFT).id).to.equal(DIRECTION_RIGHT)
        expect(router.feed(DIRECTION_RIGHT).id).to.equal(DIRECTION_LEFT)
        expect(router.feed(DIRECTION_DOWN)).to.equal(false)
        expect(router.feed(DIRECTION_DOWN_RIGHT)).to.equal(false)
        expect(router.feed(DIRECTION_RIGHT).id).to.equal(DIRECTION_LEFT)
